    ...
    async def help(self, ctx):
        ...
        if is_mod(ctx):
            msg = (f"**Commands:**\r\n"
                   ...
                   f"   Sends a message in #channel (Admins only)\r\n"
                   f"`{prefix}editMsg #channel [Message ID]:`\r\n"
                   f"   Edits the message associated with the message ID in #channel (Admins only)\r\n"
                   f"`{prefix}archive #channel:`\r\n"
                   f"   Sends a message using the archive format in #channel (Admins only)\r\n"
                   ...
                   f"   Adds the project to the <#{sea_projects_channel_id}> (SEA only, Admins only)\r\n"
                   f"`{prefix}removeProject [ProjectNumber]:`\r\n"
                   f"   Removes the object with the corresponding number from the <#{sea_projects_channel_id}> (Admins only)\r\n"
                   f"</pin:0>:\r\n"
                   f"   Pins the selected message\r\n"
                   f"</add_members:0>:\r\n"
                   f"   Adds member with {server} Role to the #thread")
